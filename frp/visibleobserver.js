 ...
 * @private
 */
recoil.frp.VisibleObserver.NodeAndValues = function() {
};

/**
 * {Node}
 **/
//recoil.frp.VisibleObserver.NodeAndValues.prototype.node = null;
...
recoil.frp.VisibleObserver = function() {
	...
	this._watched = new goog.structs.AvlTree(
			recoil.frp.VisibleObserver.WATCHED_COMPARATOR_);
	this._states = new goog.structs.AvlTree(
			recoil.frp.VisibleObserver.WATCHED_COMPARATOR_);

	this._observer = new MutationObserver(recoil.frp.VisibleObserver
			.observeFunc_(this));
...
recoil.frp.VisibleObserver.prototype.findChangedNodes_ = function (mutations) {
		var found = new goog.structs.AvlTree(
				recoil.frp.VisibleObserver.WATCHED_COMPARATOR_);
...
recoil.frp.VisibleObserver.observeFunc_ = function(me) {
	...
	return function(mutations) {
		...
		var toRemove = new goog.structs.AvlTree(
				recoil.frp.VisibleObserver.WATCHED_COMPARATOR_);
		var toAdd = new goog.structs.AvlTree(
				recoil.frp.VisibleObserver.WATCHED_COMPARATOR_);
		...
		found.inOrderTraverse(function(p) {
			...
			if (state !== null) {
				...
				var visible = exists ? recoil.frp.VisibleObserver.visible(p.node)
						: false;
				...
				if ( exists) {
					while (cur != null) {
						...
						if (index === -1) {
							...
							if (w === null) {
								...
								if (w === null) {
									toAdd.add(recoil.frp.VisibleObserver
											.createWatched_(cur, p.node));
								} else {
									w.effected.add({
										node : p.node
									});
					...
					// if the node doesn't exist we simply want to remove it otherwise we will get
					// memory leaks
					me._states.remove(p);
				}
...
recoil.frp.VisibleObserver.prototype.listen = function(node, callback) {
	...
	while (cur != null) {
		...
		if (!found) {
			this._watched
					.add(recoil.frp.VisibleObserver.createWatched_(cur, node));
		}
		cur = goog.dom.getParentElement(cur);
	}
	state = {
...
recoil.frp.VisibleObserver.createWatched_ = function(watching, effected) {
	...
	node.effected = new goog.structs.AvlTree(
			recoil.frp.VisibleObserver.WATCHED_COMPARATOR_);
...
recoil.frp.VisibleObserver.prototype._observe = function(node) {
...
recoil.frp.VisibleObserver.setUniqueDomId_ = function(node) {
	if (node.id === undefined || node.id === "") {
		node.id = "recoil.frp.id." + recoil.frp.VisibleObserver._nextDomId.toString();

		recoil.frp.VisibleObserver._nextDomId = recoil.frp.VisibleObserver._nextDomId
 ...
 * @return number of nodes we are watching  
 **/
 ...
 * 
 */
recoil.frp.VisibleObserver._nextDomId = goog.math.Long.ZERO;
