 ...
 *
 */
recoil.frp.VisibleObserver =
        function() {

            this._watched =
                    new goog.structs.AvlTree(
                            recoil.frp.VisibleObserver.WATCHED_COMPARATOR_);
            this._states =
                    new goog.structs.AvlTree(
                            recoil.frp.VisibleObserver.WATCHED_COMPARATOR_);

            this._observer =
                    new MutationObserver(recoil.frp.VisibleObserver
                            .observeFunc_(this));
 ...
 */
recoil.frp.VisibleObserver.prototype.findChangedNodes_ =
        function(mutations) {
            var found =
                    new goog.structs.AvlTree(
                            recoil.frp.VisibleObserver.WATCHED_COMPARATOR_);
            ...
            mutations
                    .forEach(function(mutation, index, array) {
                        try {
 ...
 * @return {Function=} callback for mutation observer
 * @private
 *
 */
recoil.frp.VisibleObserver.observeFunc_ =
        function(me) {
                            ...
                            if (state !== null) {
                                var exists =
                                        recoil.frp.VisibleObserver
                                                .exists(p.node);
                                var visible =
                                        exists ? recoil.frp.VisibleObserver
                                                .visible(p.node) : false;
                                    ...
                                    while (cur != null) {
                                        ...
                                        if (index === -1) {
                                            ...
                                            if (w === null) {
                                                ...
                                                if (w === null) {
                                                    toAdd
                                                            .add(recoil.frp.VisibleObserver
                                                                    .createWatched_(
                                                                            cur,
                                                                            p.node));
                                                } else {
...
/**
 * listens to node and fires callback when its visibility has changed
 * if the node is removed from the DOM it will no longer listen, also
 * the node must be in the DOM to observe
 *
 * @param {Node} node
 * @param {function(boolean)} callback
 * @throws {recoil.exception.frp.NotInDom}
 */
recoil.frp.VisibleObserver.prototype.listen =
        function(node, callback) {
            ...
            if (state !== null) {
                ...
                state.visible =
                        state.exists &&
                                recoil.frp.VisibleObserver.visible(node);
            ...
            while (cur != null) {
                ...
                this._watched.inOrderTraverse(function(travNode) {
                    if (recoil.frp.VisibleObserver.WATCHED_COMPARATOR_(
                            travNode, {
                                node: cur
                            }) === 0) {
                ...
                if (!found) {
                    this._watched.add(recoil.frp.VisibleObserver
                            .createWatched_(cur, node));
                }
                cur = goog.dom.getParentElement(cur);
            }
            state = {
            ...
            state.visible =
                    state.exists && recoil.frp.VisibleObserver.visible(node);
 ...
 * @param {Node} effected the node that the visibility of the watched
 *          node effects
 * @return {NodeAndValue}
 */
recoil.frp.VisibleObserver.createWatched_ =
        function(watching, effected) {
            var node = {
                node: watching
            };
            node.effected =
 ...
 */
recoil.frp.VisibleObserver.setUniqueDomId_ =
        function(node) {
            if (node.id === undefined || node.id === '') {
                node.id =
                        'recoil.frp.id.' +
                                recoil.frp.VisibleObserver.nextDomId_
                                        .toString();

                recoil.frp.VisibleObserver.nextDomId_ =
                        recoil.frp.VisibleObserver.nextDomId_
                                .add(goog.math.Long.ONE);
            }
            return node.id;
        };
