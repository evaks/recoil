...
recoil.structs.table.TableRow = function(opt_tableRow) {
    this.cells_ = new Map();
    this.cells_.set(recoil.structs.table.ColumnKey.ROW_META, recoil.structs.table.TableCell(undefined, {}));
    this.pos_ = undefined;
    if (opt_tableRow !== undefined) {
        for (const [key, value] of opt_tableRow.orig_) {
            this.cells_.set(key, value);
        }

        for (const [key, value] of opt_tableRow.changed_) {
            this.cells_.set(key, value);
        }
...
recoil.structs.table.TableRow.prototype.valuesEqual = function(that) {
    ...
    this.forEachColumn(function(col, cell) {
        ...
        if (!recoil.util.object.isEqual(cell.getValue(), that.cells_.get(col).getValue())) {
            equal = false;
            return true;
        }
        return false;
...
recoil.structs.table.TableRow.prototype.getCell = function(column) {
    var res = this.cells_.get(column);
...
recoil.structs.table.TableRow.prototype.getMeta = function() {
    var res = this.cells_.get(recoil.structs.table.ColumnKey.ROW_META);
...
recoil.structs.table.MutableTableRow.prototype.getCell = function(column) {
    if (this.changed_.has(column)) {
        return this.changed_.get(column);
    }
    var res = this.orig_.get(column);
    if (res === undefined) {
        return null;
    }
    return this.orig_.get(column);
};
